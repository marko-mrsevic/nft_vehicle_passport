...
contract VehicleNft is ERC721 {
    ...
    //      Events      //
    //////////////////////
    event VehicleMinted(address indexed to, uint tokenId, string vin, string uri);
    event TokenURIUpdated(uint indexed tokenId, string newURI);
    event OwnershipTransferred(uint indexed tokenId, address from, address to);
    ...
    function mintVehicleNft(string memory vin, string memory make, string memory model, string memory tokenUri) external {
